...
u1_j, u1_jp, u1_jn, u2_j, u2_jp, u2_jn = np.loadtxt(path1 + '/Results1/cal_us_and_evidance/cal_u1_u2.dat', usecols = (1,2,3,4,5,6), unpack = True)
...
for i in range(10000):
	m1 = np.array([])
	for j in range(len(diff_u1_c_ar)):
		m1 = np.hstack((m1,diff[j+1][i]))
	m2 = np.mean(m1)
	mean3r = np.hstack((mean3r,m2))
...
for i in range(10000):
	m1 = np.array([])
	for j in range(len(diff_u2_c_ar)):
		m1 = np.hstack((m1,diff[j+1][i]))
	m2 = np.mean(m1)
	mean4r = np.hstack((mean4r,m2))
...
plt.fill_between([teff[0], teff[-1]], [med1 - std1, med1 - std1], [med1 + std1, med1 + std1], color = 'red', alpha = 0.5)
...
popt, pcov = cft(line, teff, diff_u1_c_p)
popt1, pcov1 = cft(line, teff, diff_u1_c_a)

plt.plot(teff, line(teff, *popt), color='orangered', linestyle=':')
plt.plot(teff, line(teff, *popt1), color='cornflowerblue', linestyle=':')
...
popt2, pcov2 = cft(line, teff, diff_u2_c_p)
popt3, pcov3 = cft(line, teff, diff_u2_c_a)

plt.plot(teff, line(teff, *popt2), color='orangered',linestyle=':')
plt.plot(teff, line(teff, *popt3), color='cornflowerblue',linestyle=':')
...
popt4, pcov4 = cft(line, teff, diff_u1_co_p)
popt5, pcov5 = cft(line, teff, diff_u1_co_a)

plt.plot(teff, line(teff, *popt4), color='orangered',linestyle=':')
plt.plot(teff, line(teff, *popt5), color='cornflowerblue',linestyle=':')
...
popt6, pcov6 = cft(line, teff, diff_u2_co_p)
popt7, pcov7 = cft(line, teff, diff_u2_co_a)

plt.plot(teff, line(teff, *popt6), color='orangered',linestyle=':')
plt.plot(teff, line(teff, *popt7), color='cornflowerblue',linestyle=':')
